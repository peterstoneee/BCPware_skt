...
public:
  ...
  //***20150605
  virtual void executeEdit(QAction *,RichParameterSet &, bool = false){}
  //virtual void updateTransformMenu(){}
...
public:
    ...
    /// Standard stuff that usually should not be redefined.//繼承的東西不需要重複定義
    void setLog(GLLogStream *log) { this->log = log ;}
    // This fucntion must be used to communicate useful information collected in the parsing/saving of the files.//用來連結file中有用的訊息
    // NEVER EVER use a msgbox to say something to the user.//*****絕對不要使用msgbox提供使用者訊息
...
public:
  ...
  /** \brief This function is called by the framework, for each plugin that has global parameters (e.g. \ref MeshDecorateInterface) at the start of the application.
   ...
   A typical example is the background color.//每個plugin都應該有一致的全域變數，在不同的地方調用，可以維持一致性。ex. backgroundcolor
   ...
   //如果filter想要儲存某些永久的東西 
   //***plugins 如果沒有GlobalParameter，就不做任何事。
*/
...
public:
  ...
  */
    enum FilterClass
    {
		...
		MovePos			 =0x20000, //
		Customized		 =0x40000 //***20150323***新增
    };
  ...
  //filtername簡短的描述filter action 的 行為
  */
  ...
  * \sa initParameterSet
  */
  virtual bool applyFilter(QAction *   filter, MeshDocument &md, RichParameterSet & par, vcg::CallBackPos *cb,GLArea *a=0,vcg::SendBackData *sb = 0) = 0;
...
/**
//負責rendering 整個meshDocument，不只單純一個MeshModel.
...
//***20150602***開始做edit第二版
class MeshEditInterFace_v2 :public MeshCommonInterface
{
public :
	MeshEditInterFace_v2() :MeshCommonInterface(), glContext(NULL){}
	virtual ~MeshEditInterFace_v2() {}
	virtual void initParameterSet( QAction* ,MeshModel &/*m*/, RichParameterSet & /*par*/) {}
	virtual void initParameterSet( QAction *editAction,MeshDocument &md, RichParameterSet &par)
	{
		initParameterSet(editAction, *(md.mm()), par);
	}
	virtual void mousePressEvent(QMouseEvent *event, MeshModel &/*m*/, GLArea *,MeshDocument *) = 0;
	virtual void mouseMoveEvent(QMouseEvent *event, MeshModel &/*m*/, GLArea *, MeshDocument *) = 0;
	virtual void mouseReleaseEvent(QMouseEvent *event, MeshModel &/*m*/, GLArea *, MeshDocument *) = 0;
	virtual bool updateModelProjectionMatrix(GLArea *)=0;
	...
	virtual bool applyEdit_v2(QAction *, MeshDocument &md, RichParameterSet & par, vcg::CallBackPos *cb/*, GLArea *gla*/) = 0;
...
/** MeshEditInterfaceFactory
	...
	editin fileters 有內部狀態，如果一個tool要給兩個不同的文件，就需要兩個物件，每個document有獨立的interface 
 */
...
#define MESH_EDIT_INTERFACE_V2_IID  "vcg.meshlab.MeshEditInterFace_v2/1.0"
...
Q_DECLARE_INTERFACE(MeshEditInterFace_v2, MESH_EDIT_INTERFACE_V2_IID)
