...
#include "picaApplication.h"
#include "Logger_test.h"


#ifndef PALETTE_SCALAR
#error "Fatal compilation error: PALETTE_SCALAR must be defined"
#endif

typedef PALETTE_SCALAR Scalarm;
typedef vcg::Point2<PALETTE_SCALAR>   Point2m;
typedef vcg::Point3<PALETTE_SCALAR>   Point3m;
typedef vcg::Point4<PALETTE_SCALAR>   Point4m;
typedef vcg::Plane3<PALETTE_SCALAR>   Plane3m;
typedef vcg::Segment3<PALETTE_SCALAR> Segment3m;
typedef vcg::Box3<PALETTE_SCALAR>     Box3m;
typedef vcg::Matrix44<PALETTE_SCALAR> Matrix44m;
typedef vcg::Matrix33<PALETTE_SCALAR> Matrix33m;
typedef vcg::Shot<PALETTE_SCALAR>     Shotm;
...
namespace vcg
{
  namespace vertex
  {
    template <class T> class Coord3m: public Coord<vcg::Point3<PALETTE_SCALAR>, T> {
    public:	static void Name(std::vector<std::string> & name){name.push_back(std::string("Coord3m"));T::Name(name);}
    };

    template <class T> class Normal3m: public Normal<vcg::Point3<PALETTE_SCALAR>, T> {
  ...
  namespace face
  {
    template <class T> class Normal3m: public NormalAbs<vcg::Point3<PALETTE_SCALAR>, T> {
    public:  static void Name(std::vector<std::string> & name){name.push_back(std::string("Normal3m"));T::Name(name);}
    };
  }//end namespace face
}//end namespace vcg
...
public:
	...
	CoordType p;
	//texcoord
	vcg::TexCoord2<Scalarm> texC;
	//color
	vcg::Color4b pointColor;
	//tmid
...
public:
	...
	vcg::GlTrimesh<CMeshO, OutlinePointAttrib> glw;
    CMeshO cm;
	//CMeshO ccm;
	//CMeshO dcm;
    RenderMode rmm;
	//RenderMode crmm;
	//RenderMode drmm;
	//RenderMode redoTrans_rm;

	//***20150309*****加入outline參數**********************
public:
	std::vector< std::vector<CoordType> > outlines;//儲存有幾條out_line
	
	std::vector<std::vector<OutlinePointAttrib >> outlines_2;
	float outlineThick;
	bool drawCircleInQuad;
	//****************************
};
...
////rotate store structure
//
//struct mat_axis
//{
//	std::vector<std::pair<Point3<Scalarm>, Matrix33<Scalarm>>> rotate_matrix_list;
//};
...
public:
    ...
    */
	enum meshsort{ print_item, decorate_item, slice_item, cap_item, pre_print_item,temp_gen_item};
private:
    int currentDataMask;
...
public:
	...
	inline void setid(int idx){  _id = idx; }
	bool is3mf;
	...
	//***20151014
	void sethitGroove(bool h){ hitgroove = h; }
	bool gethitGroove() const{ return hitgroove; }
	//***20160304
	void setMeshSort(meshsort a){ mesh_sr = a; }
	enum meshsort getMeshSort(){ return mesh_sr; }
...
public:
	...
	std::vector<std::pair<Point3<Scalarm>, Matrix44<Scalarm>>> mesh_rotate_list;
...
public:
	...
	MeshModel *getMeshByLabel(QString name);
    MeshModel *getMeshByFullName(QString pathName);
	bool hasMesh(QString name);
	//***20150330增加名字包含字串判斷
	bool hasContainStringMesh(QString name);
	void getMeshByContainString(QString name, QVector<MeshModel *> &);
	//******************************************
	...
	//***20150813***增加回傳copyselectedMesh
	MeshModel *selCm()
	{
		return selectedCopyMesh;
	}
	void setSelectedCopyMesh(int i);
	void setSelectedCopy(MeshModel  *copyMesh)  { setSelectedCopyMesh(copyMesh->id()); }
	MeshModel * getSelectedCopyMesh(){ return selectedCopyMesh; }

	//***20160608***test_multiple_selected******************************************	
	QSet<int> *selCmeshes()//selectcopyMeshes
	{
		return &multiSelectID;
	}
	//void setSelectedCopyMesh(int i);
	void setSelectedCopies(QSet<int> copyMeshes)  { selectedCopyMeshes = copyMeshes; }
	//********************

	//*** 20170124 --- added QList copiedMeshList for storage of copied mesh model.
	// added by (R&D-I-SW, Mark)
	QList<MeshModel *> copiedMeshList;
	QList<MeshModel *> deletedMeshList;
	
	QList<int> deletedMeshId;
	QList<Color4b> oldColorList;

	//*** 20170314 added scale function
	void scaleMesh(MeshModel *mm, Point3m scaleFactor);

	//***20151014***增加回傳hitMesh
	MeshModel *hitSm()
	{
		return hitMesh;
	}
	void setHitGrooveMesh(int i);
	void setHitGroove(MeshModel  *hitMesh)  { setHitGrooveMesh(hitMesh->id()); }
	...
	QMap<int, MeshModel*>  resetMeshList;
...
public:
	...
	bool isPrint_item(MeshModel &m)
	{
		/*if (&m);
		else return false;*/
		if (m.getMeshSort() == MeshModel::meshsort::print_item || m.getMeshSort() == MeshModel::meshsort::pre_print_item)return true;
		else return false;
	}
	bool isDecorate_item(MeshModel &m)
	{
		if (m.getMeshSort() == MeshModel::meshsort::decorate_item)return true;
		else return false;
	}
	bool isSlice_item(MeshModel &m)
	{
		if (m.getMeshSort() == MeshModel::meshsort::slice_item)return true;
		else return false;
	}
	QList<MeshModel *> despiteSelectedMesh()
	{
		QList<MeshModel *> temp;
		foreach(MeshModel *mi, meshList)
		{
			if (isPrint_item(*mi) && multiSelectID.find(mi->id()) == multiSelectID.end())
				temp.push_back(mi);
		}
		return temp;
	}
	int count_print_item()
	{
		int count = 0;
		//QList<MeshModel*>::iterator mmi;

		/*foreach(MeshModel *mmi, meshList)
		{*/
		foreach(int i, multiSelectID)
		{
			MeshModel *mmi = getMesh(i);
			if (isPrint_item(*mmi))count++;
		}return count;
	}
	int count_all_item()
	{
...
public:
	...
	bool delMeshes();
	//***20150813***add
	bool copyMesh(MeshModel *mmToCopy);
	bool copyMeshes();
	//*********************************************
    ...
    int vn() /// Sum of all the vertices of all the meshes
    {
		...
		{
			//if (!(mmp->label().contains(p_setting.getOlMeshName(), Qt::CaseSensitive) || mmp->label().contains(p_setting.getCapMeshName(), Qt::CaseSensitive)))
			if(isPrint_item(*mmp))
			tot += mmp->cm.vn;
		}
        return tot;
    ...
    Box3m bbox()
    {
		...
		foreach(MeshModel * mp, meshList)
		{
			...
			//if (!(mp->label().contains(p_setting.getOlMeshName(), Qt::CaseSensitive) || mp->label().contains(p_setting.getCapMeshName(), Qt::CaseSensitive)))
			//if (mp->getMeshSort())
	...
	int selvn() /// Sum of all the vertices of all the meshes
	{
		int tot = 0;
		foreach(int i, multiSelectID)
		{
			MeshModel *mmp = getMesh(i);
			//if (!(mmp->label().contains(p_setting.getOlMeshName(), Qt::CaseSensitive) || mmp->label().contains(p_setting.getCapMeshName(), Qt::CaseSensitive)))
			if (isPrint_item(*mmp))
				tot += mmp->cm.vn;
		}
...
public:
	QSet<int> selectedCopyMeshes;
	//MeshModel *currentMesh;
private:
    MeshModel *currentMesh;
	MeshModel *selectedCopyMesh;//***20150818
	// QSet<int> selectedCopyMeshes;
    //the current raster model
	MeshModel *hitMesh;//***20151014沒用到，暫時先保留
...
signals:
	...
	//***20150514
public:
	Construction_Groove<Scalarm>  groove;// 傳值的話在meshdocuement 初始化時傳
	//***20151012***
	slice_program_setting p_setting;
	//QMap<int, QString> jobcount;
	QVector<QString> jobname;
public:
	QSet<int> multiSelectID;
	QSet<int>* getMultiSelectID(){ return &multiSelectID; }


	QVector<bool> wipeFlag;
	std::vector<vcg::Point3f> pointsInFrontOfMesh;

	//std::multimap<int, int> intersectMeshesID;
	std::set<int> intersectMeshesID2;
	std::set<int> *getIntersectMeshesID2()//selectcopyMeshes
	{
		return &intersectMeshesID2;
	}
	void setIntersectMeshesID2(std::set<int> input)//selectcopyMeshes
	{
		intersectMeshesID2.clear();
		intersectMeshesID2 = input;
	}


	//std::multimap<int, int> *getIntersectMeshesID()//selectcopyMeshes
	//{
	//	return &intersectMeshesID;
	//}
	//void setIntersectMeshesID(std::multimap<int, int> input)//selectcopyMeshes
	//{
	//	intersectMeshesID.clear();
	//	 intersectMeshesID = input;
	//}
