...
{
  pluginsDir=QDir(getDefaultPluginDirPath());//得到路徑/plugins
  // without adding the correct library path in the mac the loading of jpg (done via qt plugins) fails
...
//      qDebug() << fileName;
    ...
    {
      QPluginLoader loader(absfilepath);//絕對路徑
      QObject *plugin = loader.instance();
      if (plugin)
      {
        pluginsLoaded.push_back(fileName);//把filename存起來
        MeshFilterInterface *iFilter = qobject_cast<MeshFilterInterface *>(plugin);//object轉型成meshFilterInterface
        if (iFilter)
        {
          meshFilterPlug.push_back(iFilter);
          foreach(QAction *filterAction, iFilter->actions())//***取出action
          {
        ...
        MeshEditInterFace_v2 *iEditInterfaceV2 = qobject_cast<MeshEditInterFace_v2 *>(plugin);
        {
            if(iEditInterfaceV2)
            {				
				meshEditInterfacePlugV2.push_back(iEditInterfaceV2);
				qDebug() << "meshEditInterfacePlugV2" << meshEditInterfacePlugV2.size();
				qDebug() << "editAction" << iEditInterfaceV2->actions().size();
				foreach(QAction* editActionV2, iEditInterfaceV2->actions())
				{					
					editActionList_v2.push_back(editActionV2);
					stringEditMap_v2.insert(editActionV2->text(), iEditInterfaceV2);
				}
...
*/
//這個函式產生filtername 與空的RichParameter QMap 的關聯。
