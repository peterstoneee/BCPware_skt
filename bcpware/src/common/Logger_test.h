	...
	namespace Diagnostics
	{
		...
		public:
			CLogger(framework::Diagnostics::LogLevel level, LPCTSTR name,
				int loggableItems = static_cast<int>(LogItem::Function) | static_cast<int>(LogItem::LineNumber) | static_cast<int>(LogItem::DateTime) | static_cast<int>(LogItem::LogMemory)|
			...
			void Log(LogLevel level, LPCTSTR file, INT line, LPCTSTR func, LPCTSTR fmt, ...)
			{
				...
				for (vector<StreamInfo>::iterator iter = m_outputStreams.begin(); iter < m_outputStreams.end(); iter++)
				{
					...
					if (m_loggableItem & static_cast<int>(LogItem::LogMemory))
						written = write_system_remain_memory(written, pStream);
		...
		private:
			...
			inline bool write_datetime(bool written, wostream* strm)
			{
				...
				(*strm) << strDate << _T(" ") << strTime << _T(" ") << clock() ;

				return true;
			}
			inline bool write_system_remain_memory(bool written, wostream* strm)
			{
				if (written == true)
				{
					(*strm) << _T(" ");
