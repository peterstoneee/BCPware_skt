...
//- All the get<TYPE> are very similar. Nothing interesting here.
   ...
   Matrix44f RichParameterSet::getMatrix44(QString name) const { return findParameter(name)->val->getMatrix44f();}
     ...
     Point3<PALETTE_SCALAR> RichParameterSet::getPoint3m(QString name)  const { return Point3<PALETTE_SCALAR>::Construct(findParameter(name)->val->getPoint3f());}
...
void RichParameterCopyConstructor::visit(RichDynamicFloat_S& pd)
{
	DynamicFloatDecoration* dec = reinterpret_cast<DynamicFloatDecoration*>(pd.pd);
	lastCreated = new RichDynamicFloat(pd.name, pd.val->getDynamicFloat(), pd.pd->defVal->getDynamicFloat(), dec->min, dec->max, pd.pd->fieldDesc, pd.pd->tooltip);
}

void RichParameterCopyConstructor::visit(RichDynamicFloat_spin& pd)
{
	DynamicFloatDecoration* dec = reinterpret_cast<DynamicFloatDecoration*>(pd.pd);
	lastCreated = new RichDynamicFloat_spin(pd.name, pd.val->getDynamicFloat(), pd.pd->defVal->getDynamicFloat(), dec->min, dec->max, pd.pd->fieldDesc, pd.pd->tooltip);
}
...
//***20151014
void RichParameterXMLVisitor::visit(RichDynamicFloat_S& pd)
{
	fillRichParameterAttribute("RichDynamicFloat", pd.name, QString::number(pd.val->getDynamicFloat()), pd.pd->fieldDesc, pd.pd->tooltip);
	DynamicFloatDecoration* dec = reinterpret_cast<DynamicFloatDecoration*>(pd.pd);
	parElem.setAttribute("min", QString::number(dec->min));
...
//***20151014
void RichParameterXMLVisitor::visit(RichDynamicFloat_spin& pd)
{
	fillRichParameterAttribute("RichDynamicFloat_spin", pd.name, QString::number(pd.val->getDynamicFloat()), pd.pd->fieldDesc, pd.pd->tooltip);
	DynamicFloatDecoration* dec = reinterpret_cast<DynamicFloatDecoration*>(pd.pd);
	parElem.setAttribute("min", QString::number(dec->min));
	parElem.setAttribute("max", QString::number(dec->max));
}
...
RichMesh::RichMesh( const QString nm,int meshind,const QString /*desc*//*=QString()*/,const QString /*tltip*//*=QString()*/  )
    :RichParameter(nm,new MeshValue(NULL),new MeshDecoration(meshind))
{
    meshindex = meshind;
}
...
//***20151015****************************
RichDynamicFloat_S::RichDynamicFloat_S(const QString nm, const float defval, const float minval, const float maxval, const QString desc/*=QString()*/, const QString tltip/*=QString()*/) :RichParameter(nm, new DynamicFloatValue(defval), new DynamicFloatDecoration(new DynamicFloatValue(defval), minval, maxval, desc, tltip))
{

}

RichDynamicFloat_S::RichDynamicFloat_S(const QString nm, const float val, const float defval, const float minval, const float maxval, const QString desc/*=QString()*/, const QString tltip/*=QString()*/) : RichParameter(nm, new DynamicFloatValue(val), new DynamicFloatDecoration(new DynamicFloatValue(defval), minval, maxval, desc, tltip))
{

}

void RichDynamicFloat_S::accept(Visitor& v)
{
	v.visit(*this);
}

bool RichDynamicFloat_S::operator==(const RichParameter& rb)
{
	return (rb.val->isDynamicFloat() && (name == rb.name) && (val->getDynamicFloat() == rb.val->getDynamicFloat()));
}

RichDynamicFloat_S::~RichDynamicFloat_S()
{

}
//**********************************
RichDynamicFloat_spin::RichDynamicFloat_spin(const QString nm, const float defval, const float minval, const float maxval, const QString desc/*=QString()*/, const QString tltip/*=QString()*/) :RichParameter(nm, new DynamicFloatValue(defval), new DynamicFloatDecoration(new DynamicFloatValue(defval), minval, maxval, desc, tltip))
{

}

RichDynamicFloat_spin::RichDynamicFloat_spin(const QString nm, const float val, const float defval, const float minval, const float maxval, const QString desc/*=QString()*/, const QString tltip/*=QString()*/) : RichParameter(nm, new DynamicFloatValue(val), new DynamicFloatDecoration(new DynamicFloatValue(defval), minval, maxval, desc, tltip))
{

}

void RichDynamicFloat_spin::accept(Visitor& v)
{
	v.visit(*this);
}

bool RichDynamicFloat_spin::operator==(const RichParameter& rb)
{
	return (rb.val->isDynamicFloat() && (name == rb.name) && (val->getDynamicFloat() == rb.val->getDynamicFloat()));
}

RichDynamicFloat_spin::~RichDynamicFloat_spin()
{
