...
bool IoX3DPlugin::open(const QString &formatName, const QString &fileName, MeshModel &m, int& mask, const RichParameterSet &, CallBackPos *cb, QWidget *parent)
{
	...
	wchar_t *uniSentence = new wchar_t[sizee];
	//jpSentence->
	int y = fileName.toWCharArray(uniSentence);
	uniSentence[fileName.size()] = 0;

	FILE * pFile = _tfopen(uniSentence, _T("rt"));
	...
	/*for (int i = 0; i < fileName.length(); i++)
	{
		...
		if (isUnicode)
		{
			QMessageBox::information(NULL, tr("File name error"), tr("fileName contain unicode"));
			return false;
		}
	}*/

	/*Unicode->ANSI*/
	/*WCHAR* wc = new WCHAR[64];
	fileName.toWCharArray(wc);

	char szANSIString[MAX_PATH];
	WideCharToMultiByte(CP_ACP, WC_COMPOSITECHECK, wc, -1, szANSIString, sizeof(szANSIString), NULL, NULL);

	WCHAR szWideProgID[128];
	CLSID clsid;
	long lLen = MultiByteToWideChar(CP_ACP, 0, szANSIString, strlen(szANSIString), szWideProgID, sizeof(szWideProgID));
	szWideProgID[lLen] = '\0';*/
	...
	//if (QFile::exists();)
	//QByteArray fileName2 = fileName.toUtf8();


	//wstring filename = fileName.toWCharArray();
	...
	if (formatName.toUpper() == tr("X3D") || formatName.toUpper() == tr("X3DV") || formatName.toUpper() == tr("WRL"))
	{
		...
		if (formatName.toUpper() == tr("X3D"))
			result = vcg::tri::io::ImporterX3D<CMeshO>::LoadMask(QFile::encodeName(fileName).constData(), info);
		else
			//result = vcg::tri::io::ImporterX3D<CMeshO>::LoadMaskVrml(fileName.c_str(), info);
			result = vcg::tri::io::ImporterX3D<CMeshO>::LoadMaskVrml(uniSentence, info);
		//result = vcg::tri::io::ImporterX3D<CMeshO>::LoadMaskVrml(uniSentence, info);
		//result = vcg::tri::io::ImporterX3D<CMeshO>::LoadMaskVrml(fileName, info);
		...
		result = vcg::tri::io::ImporterX3D<CMeshO>::Open(m.cm, QFile::encodeName(fileName).constData(), info, cb);
		if (result != vcg::tri::io::ImporterX3D<CMeshO>::E_NOERROR)
		{
		...
		for (unsigned int tx = 0; tx < info->textureFile.size(); ++tx)
		{
			...
			//if (pFile == NULL)
			if (!QFile::exists(info->textureFile[tx].toStdString().c_str()))
			{
