...
namespace io {
	...
	private:
			...
			for(int in = 0; in < inlineNodes.size(); in++)
			{
				...
				if(inl.attribute("load", "true") == "true")
				{
					...
					while(j < vrmlPaths.size() && !found)
					{
						...
						TCHAR *tempTC = new TCHAR[vrmlPaths.at(j).length() + 1];
						vrmlPaths.at(j).toWCharArray(tempTC);
						tempTC[vrmlPaths.at(j).length()] = 0;
						int result = ParseVrml(tempTC, document);
						if (result == E_NOERROR)
						{
		...
		static int ManageExternProtoDeclare(QDomDocument* doc, AdditionalInfoX3D*& info, const QString filename)
		{
			...
			for(int en = 0; en < exProtoDeclNodes.size(); en++)
			{
				...
				while (t < vrmlPaths.size() && !found)
				{
					...
					TCHAR *tempTC = new TCHAR[path.length() + 1];
					path.toWCharArray(tempTC);
					tempTC[path.length()] = 0;
					int result = ParseVrml(tempTC, document);
					if (result == E_NOERROR)
					{					
						//search the ProtoDeclare node of the prototype
		...
		{
			if (mask & Mask::IOM_VERTCOLOR && HasPerVertexColor(m))
				vertex->C() = vcg::Color4b(vcg::Color4b::White);
			if (mask & Mask::IOM_VERTTEXCOORD && HasPerVertexTexCoord(m))
			{
				...
				if (!appearance.isNull())
				{
					...
					for(int i = 0; i < texture.size(); i++)
					{
						...
						if (url != "")
						{
							...
							//===================
							//****fix file name with white space
							/*QStringList paths = url.split(" ", QString::SkipEmptyParts);
							int j = 0;
							bool load = false;
									...
									CallBackPos *cb)
		{
			...
			if (!indexList.isEmpty())
			{
				...
				for (int vv = 0; vv < nVertex; vv++)
				{
					...
					if (HasPerVertexTexCoord(m) && (info->mask & vcg::tri::io::Mask::IOM_VERTCOORD))
				...
				if (geometry.tagName() == "IndexedTriangleSet")
				{
					...
					for (int ff = 0; ff < nFace; ff++)
					{
						...
						for (int tt = 0; tt < 3; tt++)
						{
							...
							//Load texture coordinate per wedge
							if (!HasPerVertexTexCoord(m) && HasPerWedgeTexCoord(m) && (info->mask & vcg::tri::io::Mask::IOM_WEDGTEXCOORD))
									...
									CallBackPos *cb)
		{
			...
			for (int i=0; i < zDimension; i++)
			{
				for (int j=0; j < xDimension; j++)
				{
					...
					//Load normal per vertex
					if (HasPerVertexNormal(m) && normalPerVertex == "true" && (info->mask & vcg::tri::io::Mask::IOM_VERTNORMAL))
						getNormal(normalList, index * 3, m.vert[index + offsetVertex].N(), tMatrix);
					//Load texture coordinate per vertex
					if (HasPerVertexTexCoord(m) && (info->mask & vcg::tri::io::Mask::IOM_VERTTEXCOORD))
				...
				for (size_t j = 0; j < objVect.size(); j++)
				{
					...
					for (int ff = 0; ff < nFace; ff++)
					{
						for (int tt = 0; tt < 3; tt++)
						{
							...
							if(HasPerWedgeTexCoord(m) && (info->mask & vcg::tri::io::Mask::IOM_WEDGTEXCOORD))
							{
								if (texCoordIndex.isEmpty())// && !HasPerVertexTexCoord(m))
		...
		//static int ParseVrml(const char * filename, QDomDocument* root/*, QString fileName = NULL*/)
		//{
		//	wchar_t *file = coco_string_create(filename);
		//	/*int x = fileName.size();
		//	wchar_t *file = new wchar_t[x+1];
		...
		static int ParseVrml(const TCHAR *filename, QDomDocument* root/*, QString fileName = NULL*/)
		{
			wchar_t *file = coco_string_create(filename);
			/*int x = fileName.size();
			wchar_t *file = new wchar_t[x+1];
...
public:	
		...
		static int Open(OpenMeshType& m, const char* filename, AdditionalInfoX3D*& info, CallBackPos *cb = 0)
		{
			...
			info->filenameStack.push_back(QString().fromLocal8Bit(filename));
		...
		static int LoadMaskVrml(const TCHAR * filename, AdditionalInfoX3D*& addinfo)
		{
			AdditionalInfoX3D* info = new AdditionalInfoX3D();
			info->filenameStack.push_back(QString::fromWCharArray(filename));
			info->mask = 0;
			info->filename = QString::fromWCharArray(filename);
			addinfo = info;
			QDomDocument* document = new QDomDocument(QString::fromWCharArray(filename));
		...
		//static int LoadMaskVrml(const char * filename, AdditionalInfoX3D*& addinfo, QString filewName = NULL)
		//{
		//	int x = filewName.size();
		//	AdditionalInfoX3D* info = new AdditionalInfoX3D();
		//	info->filenameStack.push_back(filewName);
		//	info->mask = 0;
		//	info->filename = filewName;
		//	addinfo = info;
		//	QDomDocument* document = new QDomDocument(filewName);
		//	int result = ParseVrml(filename, document, filewName);
		//	if (result != E_NOERROR)
		//	{
		//		delete document;
		//		return result;
		...
		//Get information from vrml file about the components necessary in the mesh to load the geometry
		//static int LoadMaskVrml(QString fileName, AdditionalInfoX3D*& addinfo)
		//{
		//	AdditionalInfoX3D* info = new AdditionalInfoX3D();
		//	info->filenameStack.push_back(fileName);
		//	info->mask = 0;
		...
		//	char *des;
		//	WideCharToMultiByte(CP_UTF8, 0, jpSentence, -1, des, 20, NULL, NULL);


		//	

		//	int result = ParseVrml(des, document);
		//	if (result != E_NOERROR)
		//	{
		//		delete document;
		//		return result;
