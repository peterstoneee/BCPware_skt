...
bool ExtraSampleGPUPlugin::applyFilter(QAction * a, MeshDocument & md, RichParameterSet & par, vcg::CallBackPos * /*cb*/,GLArea *gla )
{
	...
	case FP_GPU_EXAMPLE:
	{
						  ...
						  // progress = new QProgressDialog("Task in progress...", "Cancel", 0, boxy_dim,a->parentWidget());
						   ...
						   {	
							   glMatrixMode(GL_PROJECTION); glPushMatrix(); glLoadIdentity();							   
							   glOrtho(-tempgroove.groovelength / 2., tempgroove.groovelength / 2., -tempgroove.grooveheight / 2., tempgroove.grooveheight / 2., 0, -0.005);
						   ...
						   {
							   ZxGen zGG(unsigned int(width), unsigned int(height), boxy_dim + 1, md.groove.DimX());
