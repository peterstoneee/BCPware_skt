	...
	case FP_GPU_EXAMPLE:
	{
							   ...
							   //for (int i = 0; i < 1; i++)
						   {
							   ...
							   if (par.getBool("generate_zx"))
							   {
								   ...
								   capCTemp.push_back(temp); //zxzxzxzxzxzxzxz
							   }
						   ...
						   //if (par.getBool("generate_zx"))
						   //{
						   // generateZX(width, height, boxy_dim + 1, md.groove.DimX());
						   // /*ZxGen zGG(unsigned int(width), unsigned int(height), boxy_dim + 1, md.groove.DimX(), "12345");
...
void ExtraSampleGPUPlugin::generateZX(float width, float height, int pages, float grooveDimx, string full_filename)
{
