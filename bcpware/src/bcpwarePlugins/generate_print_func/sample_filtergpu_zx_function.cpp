...
void ExtraSampleGPUPlugin::initParameterSet(QAction * action, MeshDocument &md, RichParameterSet & parlst)
{
	...
	case FP_GPU_EXAMPLE:
	{
						   ...
						   parlst.addParam(new RichFloat("useless_print", 1, "useless_print_des", ""));
	...
	case FP_GPU_EXAMPLE:
	{
							   ...
							   //for (int i = 0; i < 1; i++)
						   {
							   ...
							   if (outputOL_Image)
							   {
                                   ...
                                   outlineCTemp.push_back(QImageToCvMat(image.mirrored()));
						   ...
						   /**zG = ZxGen(unsigned int(width), unsigned int(height), boxy_dim);
						   zG->sendData(outlineCTemp, capCTemp);
						   zG->close();*/
						   ZxGen zGG(unsigned int(width), unsigned int(height), boxy_dim);						   
