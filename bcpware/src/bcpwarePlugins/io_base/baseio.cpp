...
bool BaseMeshIOPlugin::open(const QString &formatName, const QString &fileName, MeshModel &m, int& mask, const RichParameterSet &parlst, CallBackPos *cb, QWidget * /*parent*/)
{
	...
	if (formatName.toUpper() == tr("PLY"))
	{
		for (int i = 0; i < fileName.length(); i++)
		{
			...
			if (isUnicode)
			{
				QMessageBox::information(NULL, tr("Filename error"), tr("Filename contain unicode"));
				return false;
			}
		}
	...
	else if (formatName.toUpper() == tr("STL"))
	{
		...
		if (!tri::io::ImporterSTL<CMeshO>::LoadMask(uniSentence, mask))
			return false;
		m.Enable(mask);

		int result = 0;
		//FILE *fp = fopen(filename.c_str(), "r");
		FILE *fp = _wfopen(uniSentence, L"r");
		fseek(fp, 0, SEEK_END);
		...
		{
			//result = tri::io::ImporterSTL<CMeshO>::Open(m.cm, filename.c_str(), mask, cb);
			result = tri::io::ImporterSTL<CMeshO>::Open(m.cm, uniSentence, mask, cb);
		}
		if (result != 0) // all the importers return 0 on success
		{
	...
	else	if ((formatName.toUpper() == tr("OBJ")) || (formatName.toUpper() == tr("QOBJ")))
	{
		...
		if (!tri::io::ImporterOBJ<CMeshO>::LoadMask(uniSentence, oi))
			return false;
		m.Enable(oi.mask);

		//int result = tri::io::ImporterOBJ<CMeshO>::Open(m.cm, filename.c_str(), oi);
		int result = tri::io::ImporterOBJ<CMeshO>::Open(m.cm, uniSentence, oi);
