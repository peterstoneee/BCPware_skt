...
: Splitter(parent)//, groove(PALETTE_SCALAR(20.0), PALETTE_SCALAR (20.0), PALETTE_SCALAR (20.0))
{
...
void MultiViewer_Container::addView(GLArea* viewer,Qt::Orientation orient)
{
  ...
  if (viewerCounter()==1)
  {
    ...
    if(this->orientation()== Qt::Horizontal){
      ...
      //sizes.append(this->width()/2);
		sizes.push_back(500);//
		sizes.push_back(1400);//要設為總為1920的大小，不然size無法控制
    }
    else{
  ...
  // Generic Case: Each splitter Has ALWAYS two children.一般case
  viewerList.append(viewer);


  //***2015_2_6 修改為從左邊加入，方法為選取Viewer的第一個來分割
  //GLArea* currentGLA = this->currentView();
  GLArea* currentGLA = this->getViewer(0);
  //currentGLA->setStyleSheet("QWidget:focus {  border: 3px solid green;}");
  //*****************************************
  ...
  newSplitter->addWidget(viewer);//newSplitter的child
  newSplitter->addWidget(currentGLA);//newSplitter的child
  ...
  //currentSplitter->setSizes(parentSizes);//parentSizes好像抓的不對，不要再重設parentSize
...
void MultiViewer_Container::updateCurrent(int current){
	...
	{
		//***20150402
		/*GLArea *gla = getViewer(previousCurrentId);
		gla->updateTexture();*/
		//******
...
void MultiViewer_Container::closeEvent(QCloseEvent *event)
{
	...
	{
		QMessageBox::StandardButton ret = QMessageBox::question(
			this, tr("BCPware"), tr("Do you want to save changes on '%1'?").arg(meshDoc.docLabel().section('/', -1)),
		...
		if (ret == QMessageBox::Yes)
		{
			...
			if (fileInfo.exists())
			{
				...
				bool ret = tmf->save_3mf(&meshDoc, (const wchar_t*)fileInfo.absoluteFilePath().utf16(), mw->QCallBack);
			...
			else
			{
				...
				if (result == QFileDialog::Accepted)
				{
					...
					ThreeMF_Manip *tmf = new ThreeMF_Manip();
					bool ret = tmf->save_3mf(&meshDoc, (const wchar_t*)fileName.utf16(), mw->QCallBack);
...
void MultiViewer_Container::initMeshTexture(bool reloadAllTexture)
{
	

}
void MultiViewer_Container::updateAllViewerRenderModeMap()
{
	foreach(GLArea* viewer, viewerList)
	{
		viewer->rendermodemap = currentView()->rendermodemap;
	}	
}