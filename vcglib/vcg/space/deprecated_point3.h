...
//@{
  ...
  inline Point3 Perpendicular()
  {
	  ...
	  return Point3(-_v[1], _v[0], 0).Normalize();
  }
//@}
...
//@{
  ...
  **/
	...
	Point3 MinP3(Point3 b)
	{
		return Point3(std::min(this->X(), b.X()), std::min(this->Y(), b.Y()), std::min(this->Z(), b.Z()));
	}

	Point3 MaxP3(Point3 b)
	{
		return Point3(std::max(this->X(), b.X()), std::max(this->Y(), b.Y()), std::max(this->Z(), b.Z()));
	}
