            ...
            */
            ...
            public:
                ...
                public:
                    ...
                    // it returns the current position, and formats a string with a description of what the function is doing (loading vertexes, faces...)
                    CallBackPos *cb;
                ...
                */
				...
				static int Open(OpenMeshType &m, const TCHAR * filename, Info &oi)
                {
                        ...
                        if (numTokens > 0)
                        {
                            ...
                            if (header.compare("v")==0)	// vertex
                            {
                                ...
                                if (((oi.mask & vcg::tri::io::Mask::IOM_VERTCOLOR) != 0) && (HasPerVertexColor(m)))
                                {
                                    ...
                                    {
										/*if (currentColor.Red == 255 && currentColor.Green == 255 && currentColor.Blue == 255)
											;
										else
                            ...
                            else if (header.compare("vt")==0)	// vertex texture coords 有抓到vt header 與之下的座標
                            {
...
//											qDebug("ff.n[iii]=indexNVect[ locInd[iii] ]; %i", ff.n[iii]);
                            ...
                            else if ((header.compare("mtllib")==0) && (tokens.size() > 1))	// material library
                            {
								...
								{
									qDebug() << QString::fromStdString(tokens[i]);
									if (tokens[i].find(".mtl") != std::string::npos)
									{
										materialFileName.append(tokens[i]);										
									}
									else
									{
										materialFileName.append(tokens[i]);
										materialFileName.append(" ");
									}
					...
					//這邊沒有把texcoord傳回mesh，mesh.texcoord抓不到
                    for(int i=0; i<numTriangles; ++i)
                    {
                        ...
                        for(unsigned int j=0;j<indexedFaces[i].v.size();++j)
                        {
                            ...
                            if (((oi.mask & vcg::tri::io::Mask::IOM_WEDGTEXCOORD) != 0) && (HasPerWedgeTexCoord(m)))//這邊沒有把texcoord傳回mesh，mesh.texcoord抓不到
                            {
								...
								//if (m.face[i].V(j)->T().u()>0)std::cout << "m.face uv" << m.face[i].V(j)->T().u() << m.face[i].V(j)->T().v() << std::endl;
                            }
...
//							  qDebug("XXXXXX %i",indexedFaces[i].n[j]);
                ...
                */
                inline static void TokenizeNextLine(std::ifstream &stream, std::vector< std::string > &tokens, std::vector<Color4b> *colVec)
                {
                    ...
                    size_t from		= 0;
                    size_t to			= 0;
					size_t getbackhanddash = 0;
                    size_t length = line.size();

                    tokens.clear();
                    do
                    {
						while (from != length && (line[from] == ' ' || line[from] == '\t' || line[from] == '\r' || line[from] == '\\'))
						{
							from++;
							getbackhanddash = from;
						}
						...
						//suppport this format
						/*
							vn - 0.1545637845993042 - 0.0003166032256558538 \
							- 0.9879827499389648
							*/
						if (getbackhanddash == length && line[length - 1] == '\\')
						{
							std::getline(stream, line);
							from = 0;
							to = 0;
...
//					qDebug("%s -> %i %i %i",token.c_str(),vId,nId,tId);
                ...
                */
                ...
                static bool LoadMask(const TCHAR * filename, Info &oi)
                {
                    ...
                    {
						//給mask數值
                        if (oi.numTexCoords==oi.numVertices)  oi.mask |= vcg::tri::io::Mask::IOM_VERTTEXCOORD;
						//std::cout << "oi.IOM_VERTTEXCOORD" << oi.mask << std::endl;
                        oi.mask |= vcg::tri::io::Mask::IOM_WEDGTEXCOORD;
						//std::cout << "oi.IOM_WEDGTEXCOORD" << oi.mask << std::endl;
                        // Usually if you have tex coords you also have materials
                        oi.mask |= vcg::tri::io::Mask::IOM_FACECOLOR;
						//std::cout << "oi.mask" << oi.numTexCoords << std::endl;
                    }
                ...
                static bool LoadMaterials(const char * filename, std::vector<Material> &materials, std::vector<std::string> &textures)
                {
                    ...
                    while (!stream.eof())
                    {
                        ...
                        if (tokens.size() > 0)
                        {
                            ...
                            else if( (header.compare("map_Kd")==0)	|| (header.compare("map_Ka")==0) ) // texture name 材質名稱
                            {
                                if (tokens.size() < 2)
                                    return false;
								...
								for (int i = 1; i < tokens.size(); i++)
								{
									if (tokens[i].find(".jpg") != std::string::npos || tokens[i].find(".png") != std::string::npos || tokens[i].find(".BMP") != std::string::npos || tokens[i].find(".bmp") != std::string::npos || tokens[i].find(".jpeg") != std::string::npos)
									{
										textureName.append(tokens[i]);
									}
									else
									{
										textureName.append(tokens[i]);
                                ...
                                if (!found) 
                                {
									...
									//cout << j << endl;
									//std::cout << textures[j]<<"currentMaterial.index=" << currentMaterial.index << std::endl;
                                }
                            }
                            // we simply ignore other situations
                        }
					...
					std::cout <<"material"<< materials[0].index<< std::endl;
