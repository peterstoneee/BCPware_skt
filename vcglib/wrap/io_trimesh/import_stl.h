...
static bool LoadMask(const TCHAR* filename, int &mask)
{
  bool magicMode;
...
static bool IsSTLColored(const TCHAR * filename, bool &magicsMode)
{
...
static bool IsSTLBinary(const TCHAR * filename)
{
  bool binary=false;
...
static int Open(OpenMeshType &m, const TCHAR * filename, int &loadMask, CallBackPos *cb = 0)
{
 // FILE *fp = fopen(filename, "r");
	FILE *fp = _wfopen(filename, L"rb");
  if(fp == NULL)
...
static int OpenBinary(OpenMeshType &m, const TCHAR * filename, int &loadMask, CallBackPos *cb = 0)
{
  FILE *fp;
  ...
  //static int OpenAscii( OpenMeshType &m, const char * filename, CallBackPos *cb=0)
	static int OpenAscii(OpenMeshType &m, const TCHAR * filename, CallBackPos *cb = 0)
  {
    FILE *fp;
    //fp = fopen(filename, "r");
	fp = _wfopen(filename, L"rb");
