...
namespace vcg {
  ...
  //***20151006***增加畫box的函式
  template <class T>
  inline void glBoxWire_SKT3(Box3<T> const & b, bool Is_hit)
  {
	  ...
	  if (Is_hit){
		  glColor(vcg::Color4b::LightRed);
		  glLineWidth(5);
	  }
	  else{
		  glColor(vcg::Color4b::Black);
		  glLineWidth(1);
	  }
	 /* glBegin(GL_LINE_STRIP);
	  ...
	  glEnd();
	  //glClearColor(vcg::Color4b::White);
	  //glColor(vcg::Color4b::White);
	  //glLineWidth(1);
	  glPopAttrib();
  };
  ...
  //***20151006***增加畫box的函式
  template <class T>
  inline void glBoxWire_SKT(Box3<T> const & b,bool Is_hit)
  {
	  glPushAttrib(GL_ALL_ATTRIB_BITS);
	  glEnable(GL_LINE_SMOOTH);
	  glEnable(GL_POLYGON_SMOOTH);
  ...
  template <class T>
  inline void glBoxWire_SKT2(Box3<T> const & b,vcg::Color4f colorin = vcg::Color4f(1, 0, 1,0),bool drawpoint = false,int linewidth = 2)
  {
	  glPushAttrib(GL_ALL_ATTRIB_BITS);
	  glEnable(GL_POLYGON_SMOOTH);
	  glHint(GL_LINE_SMOOTH_HINT, GL_NICEST);
