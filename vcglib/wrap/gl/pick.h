 ...
 static CoordType Proj(const Eigen::Matrix<ScalarType,4,4> &M, const ScalarType * viewport, const CoordType &p)
  {
    ...
    const ScalarType vw2=viewport[2]/ScalarType(2.0);//400/2=200
  ...
  static Box3<ScalarType> ComputeDCBox(int x, int y, int width, int height)
  {
    Box3<ScalarType> bb;
    bb.SetNull();
    bb.Add(CoordType(x-width/ScalarType(2.0),y-height/ScalarType(2.0),ScalarType(-1.0)));//boundingbox 加入點
    bb.Add(CoordType(x+width/ScalarType(2.0),y+height/ScalarType(2.0), ScalarType(1.0)));//boundingbox 加入點
    return bb;
  }

public:
	static bool screenToObject(int x, int y, float*xyz)
	{
		Eigen::Matrix4d mp, mm;
		GLint viewport[4];
		glGetIntegerv(GL_VIEWPORT, viewport);
		...
		GLdouble t;
		realy = y;
		gluUnProject((GLdouble)x, (GLdouble)realy, 0.0, mm.data(), mp.data(), viewport, &wx0, &wy0, &wz0);
		gluUnProject((GLdouble)x, (GLdouble)realy, 1.0, mm.data(), mp.data(), viewport, &wx1, &wy1, &wz1);
		t = wy0 / (wy0 - wy1);
		xyz[0] = (1 - t)*wx0 + t*wx1;
		xyz[2] = (1 - t)*wz0 + t*wz1;
		xyz[1] = 0.0;
	...
	static bool MouseInBox(int x, int y, MESH_TYPE &m, std::vector<vcg::Point3f> vector)
	{
		...
		CoordType mm_max = Proj(M, viewportF, vector[0]);
		CoordType mm_min = Proj(M, viewportF, vector[0]);
		for (int i = 0; i < 8; i++)//找出bounding box投影在螢幕座標最大與最小的x y 值
		{
			if (Proj(M, viewportF, vector[i]).X()> mm_max.X())  mm_max.X() = Proj(M, viewportF, vector[i]).X();
			if (Proj(M, viewportF, vector[i]).Y()> mm_max.Y())  mm_max.Y() = Proj(M, viewportF, vector[i]).Y();
			if (Proj(M, viewportF, vector[i]).X()< mm_min.X())  mm_min.X() = Proj(M, viewportF, vector[i]).X();
			if (Proj(M, viewportF, vector[i]).Y()< mm_min.Y())  mm_min.Y() = Proj(M, viewportF, vector[i]).Y();
		}

		//CoordType m_max = Proj(M, viewportF, m.bbox.max);//bounding box max in windows coordinate
		//CoordType m_min = Proj(M, viewportF, m.bbox.min);//bounding box max in windows coordinate
		CoordType xy_windw(x, y, 0);
		mm_max.Z() = 1.0;
		mm_min.Z() = -1.0;
		Box3<ScalarType> bb;
		bb.SetNull();
		bb.Add(mm_max);//boundingbox 加入點
		bb.Add(mm_min);//boundingbox 加入點

		if (bb.IsIn(xy_windw))
		{
			//std::cout << "in" << std::endl; // std::cout << m_min.x << std::endl;
			return true;
		}
		else return false;
		//std::cout << m_max.X() <<", "<<m_max.Y()<< std::endl;
		//std::cout << m_min.X() << ", " << m_min.Y() << std::endl;

	}
	static bool MouseInBox_old(int x, int y, MESH_TYPE &m, Box3<ScalarType> copy)
	{
		...
		CoordType mm_max = Proj(M, viewportF, m.bbox.P(0));
		CoordType mm_min = Proj(M, viewportF, m.bbox.P(0));
		for (int i = 0; i < 8; i++)//找出bounding box投影在螢幕座標最大與最小的x y 值
		{
			if (Proj(M, viewportF, m.bbox.P(i)).X() > mm_max.X())  mm_max.X() = Proj(M, viewportF, m.bbox.P(i)).X();
			if (Proj(M, viewportF, m.bbox.P(i)).Y() > mm_max.Y())  mm_max.Y() = Proj(M, viewportF, m.bbox.P(i)).Y();
			if (Proj(M, viewportF, m.bbox.P(i)).X() < mm_min.X())  mm_min.X() = Proj(M, viewportF, m.bbox.P(i)).X();
			if (Proj(M, viewportF, m.bbox.P(i)).Y() < mm_min.Y())  mm_min.Y() = Proj(M, viewportF, m.bbox.P(i)).Y();
		}
  ...
  static bool PickClosestFace(int x, int y, MESH_TYPE &m, FacePointer &fp,int width=4, int height=4)
  {
    Eigen::Matrix<ScalarType,4,4> M;
    ScalarType viewportF[4];
	glGetMatrixAndViewport(M, viewportF); //std::cout <<viewportF[0]<<" "<<viewportF[1]<<" "<<viewportF[2] <<" "<< viewportF[3] << std::endl;
	Box3<ScalarType> reg = ComputeDCBox(x, y, width, height); //在螢幕座標上做BBOX,2D的bounding box

    ScalarType bzmin = std::numeric_limits<ScalarType>::max();
    fp=0;
    for(size_t i=0;i<m.face.size();++i) if(!m.face[i].IsD())
    {
      CoordType bary = vcg::Barycenter(m.face[i]);//三角形重心
	  CoordType bz = Proj(M, viewportF, bary);//將重心投在螢幕座標上,x:0~400, y:0~400, z:-1~+1

      if(bz[2]<bzmin && reg.IsIn(bz))//找出重心z值在reg bbox 裡的面且距離最短 ，bz~[-1~+1],
      {
        bzmin=bz[2];
        fp = &m.face[i];
      }
